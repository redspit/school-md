...
const SingleTeacherPage = ({ params }) => {
	...
	useEffect(() => {
		setTeacher(teachersData.find((t) => t.teacherId === teacherId))
	}, [])
	if (teacher === null) return <LoadingScreen />
	// if (teacher === undefined)  return <NotFound />
	if (teacher === undefined)  return <IdNotFound label="teacher"/>
	return (
		<div className="flex-1 flex justify-between md:flex-row flex-col gap-2 overflow-auto">
			<div className="md:w-2/3 w-full flex flex-col gap-2">
				...
				<div className="flex-1 rounded-md bg-white p-2 min-h-[500px]">
					<div className="font-bold text-sm py-2">Teacher's Schedule</div>
					<Calendar
						localizer={localizer}
			...
			<div className="md:w-1/3 flex-1 bg-transparnet rounded-md flex flex-col gap-2">
				<div className="bg-white rounded-lg p-3">
					...
					<div className="flex flex-wrap gap-1 text-[.7rem]">
						...
						<div className="bg-red-100 rounded-lg p-2">Teacher's Exams</div>
						<div className="bg-purple-100 rounded-lg p-2">
							Teacher's Assignmrnts
						</div>
					</div>
				</div>
				<div className="bg-white rounded-lg p-3">
